...
try:
    ...
    print("❌ Error: 'svc.pkl' not found!")
    model = None

# List of all 132 symptoms
symptoms = [
    ...
    "increased_appetite", "polyuria", "family_history", "rusty_sputum",
...
def survey():
    if request.method == 'POST':
        print(f"📩 Survey Data: {request.form}")
        return render_template('survey.html', symptoms=symptoms, message="Form submitted successfully!")
    return render_template('survey.html', symptoms=symptoms)

@app.route('/predict', methods=['POST'])
def predict():
    if model is None:
        return render_template("result.html", error="Model not found.")

    data = request.form.to_dict()
    features = [int(data.get(symptom, 0)) for symptom in symptoms]

    if len(features) != 132:
        return render_template("result.html", error="Invalid number of symptoms provided.")

    prediction = model.predict(np.array(features).reshape(1, -1))[0]

    disease_data = {
        "description": "A common illness affecting health.",
        "precautions": ["Stay hydrated", "Take rest", "Consult a doctor"],
        "medications": ["Paracetamol", "Antibiotics"],
        "diet": ["Fruits", "Vegetables", "Plenty of fluids"],
        "workout": ["Light stretching", "Walking"]
    }
